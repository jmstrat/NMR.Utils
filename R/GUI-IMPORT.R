interactive_import_mod_UI <- function(id) {
  ...
  shiny::fluidPage(jms.classes::jsCodeHandler(),
    ...
    NotificationStyle(),
    shiny::fluidRow(shiny::column(10, "Path to the acquisition directory (i.e. the directory containing acqus, pdata, ...)")),
    shiny::fluidRow(
    ...
    shiny::fluidRow(shiny::column(10, "Path to the ATMC log file. Optional. Used to determine the time each scan took place.")),
    shiny::fluidRow(
    ...
    shiny::fluidRow(
      shiny::column(6,
                    shiny::fluidRow(style='padding-top: 20px;',
                                    shiny::column(
                                      6,
                                      shiny::textInput(ns('max_scans'), 'Number of last scan performed', placeholder = 'Leave blank to determine automatically')
                                    ),
                                    shiny::column(
                                      6,
                    ...
                    shiny::fluidRow(style='padding-top: 20px;',
                                    shiny::column(12,shiny::div("You typically want the data range for processing to be slightly larger than what you will use in the final plot, if there is a much larger range than will be plotted, you should reduce it here to simplify processing.",
                                                                style="text-align: justify; width: calc(50% + 300px); max-width: 100%;"))
                    ),

                    shiny::fluidRow(style='padding-top: 10px;',
                                    shiny::column(
                                      6,
      ...
      shiny::fluidRow(style='padding-top: 20px;',align='center',
                      shiny::column(3, shiny::plotOutput(ns('nmr_plot'), height='250px')),
                      shiny::column(3, shiny::plotOutput(ns('echem_plot'), height='250px'), shiny::uiOutput(ns('capacitytimeUI')))
      )
    ),
    shiny::fluidRow(align='center',style='padding: 20px;',
                    shiny::column(12,
...
interactive_import_mod <- function(input, output, session, data_name = function() {'insitu_nmr_data'}, ...) {
  ...
  if(requireNamespace("Echem.Data", quietly=TRUE)) {
    ...
    output$echemFileUI <- shiny::renderUI({
      shiny::tagList(
        ...
        shiny::fluidRow(
          shiny::column(10,shiny::textInput(session$ns("echem_text"), label=NULL, placeholder = "Path to echem file", width='100%')),
          shiny::column(2,jms.classes::fileChooserUI(session$ns('echem'),label="Select File",title='Please choose the echem file',multiple=FALSE,style="width:100%; margin-top: 0px;"))
        )
      )
    })
  }
  ...
  output$capacitytimeUI <- shiny::renderUI({
    if(requireNamespace("Echem.Process", quietly=TRUE)) {
      jms.classes::toggleInput(session$ns('capacitytime'), '', FALSE, 'Cap', 'Time', style='margin-top: -25px; margin-left: 0.75cm;')
    }
  })

  output$echemProcessUI <- shiny::renderUI({
    if(is.null(input$capacitytime) || !input$capacitytime) return()
    if(requireNamespace("Echem.Database", quietly=TRUE) && (input$echem_text == "" || !suppressWarnings(is.na(as.numeric(input$echem_text))))) {
      return()
    }
    shiny::tagList(
      shiny::fluidRow(
        shiny::column(
          12,
          shiny::div("Enter the mass in order to normalise the capacity. The theoretical capacity for the material is only used for the calculation of some additional metadata.",
                     style="text-align: justify; width: calc(50% + 300px); max-width: 100%;")
        )),
      shiny::fluidRow(
        shiny::column(6, shiny::numericInput(session$ns('mass'), 'Characteristic Mass (mg)', 1, min=0)),
        shiny::column(6, shiny::numericInput(session$ns("tcap"), htmltools::HTML("Capacity of Material (mAhg<sup>-1</sup>)"), 1, 0)
        )
      )
    )
  })
  ...
  shiny::observeEvent(input$save, {assign(data_name(), imported_data_list$data, envir=.GlobalEnv)})

  shiny::observeEvent(input$copy_script, {jms.classes::clipboard_copy(imported_data_list$script_input)})
  ...
  do_import <- function(nmr_file, atmc_file, echem_file, max_scans, lower, upper, offset_hours, mass, tcap, capacitytime) {
    shiny::withProgress(message = 'Importing data', value = 1, {
      ...
      if(!grepl('/[0-9]*/?$', nmr_file)) {
        shiny::showNotification(shiny::div(htmltools::HTML('<b>Warning: path does not appear to be an acquisition directory</b><br/>Acquisition directories should end with a number. Attempting to continue anyway...')), duration = 10,
      ...
      shiny::withProgress(message = 'Importing NMR', value = 1, {
        data <- tryCatch({
          ...
          shiny::showNotification(shiny::div(htmltools::HTML('<b>Error: Could not load data!</b><br/>', e$message)), duration = 10,
        ...
        if(!is.nmr2d.data.object(data)) {
          shiny::showNotification(shiny::div(htmltools::HTML('<b>Error: Data is not 2D!</b><br/>')), duration = 10,
                                  type = "error", id='error', session = session)
          return()
        }
        ...
        if(xl1 && xl2) {
          if(lower > upper) {
            shiny::showNotification(shiny::div(htmltools::HTML('<b>Warning: ppm limits are invalid!</b><br/>Switching upper and lower limits and attempting to continue...')), duration = 10,
                                    type = "warning", id='error', session = session)
            temp <- lower
            lower <- upper
            upper <- temp
          ...
          script = sprintf("%1$s%2$s = %2$s[%2$s$ppm > %3$s & %2$s$ppm < %4$s,]\n",
                           script, data_name(), as.numeric(lower), as.numeric(upper))
        } else if(xl1) {
          data = data[data$ppm > as.numeric(lower),]
          script = sprintf("%1$s%2$s = %2$s[%2$s$ppm > %3$s,]\n",
        ...
        if(atmc_file != '') {
          ...
          if(length(offsets) < ncol(data) - 1) {
            shiny::showNotification(shiny::div(htmltools::HTML('<b>Warning: ATMC log file does not contain enough times</b><br/>')), duration = 10,
                                    type = "warning", id='error', session = session)
          }
          ...
          shiny::updateTextInput(session, 'offset_hours', placeholder = sprintf('Average (ATMC) = %s hours', round(mean(diff(offsets)), 2)))
      ...
      if(is.null(data) || nrow(data)==0) {
        shiny::showNotification(shiny::div(htmltools::HTML('<b>Error: No NMR Data!</b><br/>')), duration = 10,
                                type = "error", id='error', session = session)
        return()
      }
      ...
      if(requireNamespace("Echem.Data", quietly=TRUE) && echem_file != '') {
        shiny::withProgress(message = 'Importing echem', value = 1, {
          tryCatch({
            if(!file.exists(echem_file) && requireNamespace("Echem.Database", quietly=TRUE) && !suppressWarnings(is.na(as.numeric(echem_file)))) {
            ...
            shiny::showNotification(shiny::div(htmltools::HTML('<b>Error: Could not load echem data!</b><br/>', e$message)), duration = 10,
                                    type = "error", id='error', session = session)
            return()
          })
        })
      ...
      imported_data_list$nmr_paths <- c(acqu=input$nmr_acqu_text,
  ...
  output$echem_plot <- shiny::renderPlot({
    ...
    if(is.null(echem)) {
      ...
      if(ncol(nmr) > 1) {
        ...
        Plotting.Utils::boxed.labels(time, v, w, col=rgb(1, 0.27, 0, 1), bg=rgb(1,1,1,0.5), border=F, xpd=NA)
  ...
  shiny::callModule(data_info_mod, "info", shiny::reactive({imported_data_list$data}))
  ...
  list(
    data = shiny::reactive({imported_data_list$data}),
    data_filename = data_filename,
    script = shiny::reactive({imported_data_list$script_input}),
    packages = shiny::reactive({imported_data_list$packages}),
    action='Importing'
  )
}
