...
about_mod_UI <- function(id) {
  ...
  shiny::fluidPage(
    ...
    shiny::div(class='about-section',
               shiny::p('Wizard for processing and ploting operando / in situ NMR experiments. Data can be imported, phased, baseline corrected, and plotted. The result can be saved as a PNG image, and / or an R script that will output the image and has comments indicating what it is doing, and where it can be customised. Examples of plots that can be produced with the wizard can be found in the papers referenced below.'),
               shiny::tags$table(class='about-author',
                                 shiny::tags$tr(
                                 ...
                                 shiny::tags$tr(
                                   ...
                                   shiny::tags$td(shiny::a(htmltools::HTML('Stratford, J. M.; Allan, P. K.; Grey, C. P. <i>et al.</i> <i>Chem. Commun.</i> <b>2016</b>, <i>52</i>, 12430–12433.'), href='https://dx.doi.org/10.1039/C6CC06990H'))
                                 ),
                                 shiny::tags$tr(
                                   shiny::tags$td(),
                                   shiny::tags$td(shiny::a(htmltools::HTML('Stratford, J. M.; Mayo, M.; Allan, P. K.; Grey, C. P. <i>et al.</i> <i>J. Am. Chem. Soc.</i> <b>2017</b>, <i>139</i>, 7273–7286.'), href='https://dx.doi.org/10.1021/jacs.7b01398'))
                                 )
               )
    ),
    ...
    shiny::div(
      ...
      shiny::h4('Changing any of these settings requires the wizard to be restarted before they will take effect.'),

      shiny::h3('Logging'),
      shiny::p('This setting controls the amount of detail in the log which is output to the console whilst the wizard is active. During normal use this can be safely set to a low level (e.g. warning) or disabled, however, if you encounter a problem you should set this to Debug to ensure the greatest level of detail.'),
      shiny::uiOutput(ns('settings_log')),

      shiny::h3('Saving'),
      shiny::span('Warning: this is still under active development and has not been thoroughly tested.'),
      shiny::br(),
      shiny::p('Enabling this will add "Save" and "Load" buttons to the navigation bar, which allow you to save and restore the current state of the wizard. This is distinct from the script that can be saved in the export tab, which cannot be loaded back into the interface.'),
      ...
      shiny::fluidRow(
        ...
        shiny::column(10, shiny::div('This controls the default zoom level of the wizzard (%).', style='margin-top: 25px;'))
      ),
      shiny::span('Warning: Setting this to any value other than 100% can cause issues with interactive plots.')
    )
  )
}
...
about_mod <- function(input, output, session) {
  ...
  output$optional_echem <- shiny::renderUI({
    optional_ui('Electrochemistry', 'Support for plotting electrochemistry data aligned to the NMR plot. Biologic, Arbin, Ivium, Maccor, and Lahne data files are supported.', 'Echem.Data')
  })

  output$optional_process <- shiny::renderUI({
    optional_ui('Processing Electrochemistry', 'Support for processing electrochemistry data. Allows replacing the time axis with capacity.', 'Echem.Process')
  })

  output$optional_database <- shiny::renderUI({
    optional_ui('Electrochemistry Metadata', 'Support for fetching the path for the electrochemistry data, and the parameters necessary for processing from a database.', 'Echem.Database')
  })
  ...
  output$settings_log <- shiny::renderUI({
    ...
    shiny::selectInput(session$ns('loglevel'), '', c('Debug', 'Info', 'Warning', 'Error', 'Disabled'), selected=loglevel_map[[logLevel]])
  })
  ...
  output$settings_save <- shiny::renderUI({
    ...
    jms.classes::toggleInput(session$ns('saving'), '', enabled, offlabel='Off', onlabel='On')
  })
  ...
  zoom_r <- shiny::reactive({input$zoom})
  zoom_d <- shiny::debounce(zoom_r, 1000)
  shiny::observe({
    if(!is.numeric(zoom_d())) return()
  ...
  output$debug_log <- shiny::downloadHandler(
    ...
    content = function(file){
      ...
      if(!file.exists(logFile)) {
        shiny::showNotification(shiny::div(htmltools::HTML('<b>Error: Could not find debug log!</b><br/>Enable debug mode and try again.')), duration = 10,
