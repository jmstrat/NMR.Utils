oneD_spectra_plot_mod_UI <- function(id) {
  ...
  shiny::tagList(
    ...
    shiny::selectizeInput(ns("colours"), "Plot colours", multiple=TRUE,choices = colours()),
    shiny::fluidRow(
      shiny::column(
        6,
...
oneD_spectra_plot_mod <- function(input, output, session, data, data_name) {
  ...
  selected_plot_col_numbers <- shiny::reactive({
    ...
    if (is.null(selectedPlots)) return()
  ...
  output$xlim1UI <- shiny::renderUI({
    shiny::numericInput(session$ns('xlim1'), 'Left axis limit', xrange()[[1]], 0, step=0.05, width='100%')
  })

  output$xlim2UI <- shiny::renderUI({
    shiny::numericInput(session$ns('xlim2'), 'Right axis limit', xrange()[[2]], 0, step=0.05, width='100%')
  })

  output$ylim1UI <- shiny::renderUI({
    shiny::numericInput(session$ns('ylim1'), 'Lower y limit', yrange()[[1]], 0, step=0.05, width='100%')
  })

  output$ylim2UI <- shiny::renderUI({
    shiny::numericInput(session$ns('ylim2'), 'Upper y limit', yrange()[[2]], 0, step=0.05, width='100%')
  })

  output$selectizeUI <- shiny::renderUI({
    shiny::selectizeInput(session$ns('selectize'), 'Scans to plot', choices = 1:(ncol(data())-1), multiple = TRUE)
  })


  plot_colours <- shiny::reactive({
    nmrdata <- selected_data_with_offsets()
    if(is.null(nmrdata) || length(nmrdata) == 0) return()
    n = ncol(nmrdata)-1
    if(n == 0) return()
  ...
  plotFunc <- function() {
    ...
    if(input$showppm) {
      plot(nmrdata[,1:2], ylim=r, xlim=c(input$xlim1, input$xlim2), col=cols[[1]], div=5)
    } else {
      plot(nmrdata[,1:2], ylim=r, xlim=c(input$xlim1, input$xlim2), col=cols[[1]], axes=F)
    }
