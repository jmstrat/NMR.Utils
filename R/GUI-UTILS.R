NotificationStyle <- function() {
  shiny::tagList(
    shiny::tags$head(shiny::tags$style(".shiny-notification {height: 50px; width: 400px;} #shiny-notification-panel {top: 0; width: 400px; right: 5px;}")),
    shiny::tags$head(shiny::tags$style(".shiny-notification-error {position: fixed; top: 5%; left: 0; right: 0; margin: auto; height: 200px; opacity: 1; text-align: center;}")),
    shiny::tags$head(shiny::tags$style(".shiny-notification-warning {position: fixed; top: 5%; left: 0; right: 0; margin: auto; height: 200px; opacity: 1; text-align: center;}"))
  )
}
...
gui_try_show_error <- function(expr, message_verb) {
  jms.classes::tryCatchST(expr, error = function(e, st) {
    ...
    shiny::showNotification(shiny::div(htmltools::HTML('<b>Error: Something went wrong whilst ',message_verb,':</b><br/>', e$message)), duration = 10,
                            type = "error", id='error')
    # stop(e)
    return()
  })
