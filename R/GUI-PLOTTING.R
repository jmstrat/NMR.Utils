  ...
  shiny::fluidPage(shiny::tags$script(shiny::HTML('Shiny.addCustomMessageHandler("jsCode",function(message) {eval(message.code);});')),
                   ...
                   shiny::sidebarLayout(
                     ...
                     shiny::mainPanel(
                       ...
                       shiny::div(style='margin-top:20px; margin-bottom: 10px; text-align: justify;','The preview displayed before adding may look different under some circumstances.'),
                       shiny::fluidRow(
...
interactive_plotting_help <- function() {
  shiny::modalDialog(style="text-align: justify;",
    title = "Help",
    size = 'l',
    easyClose = TRUE,
    shiny::p('The following steps describe a sensible approach to navigate the plotting interface:'),
    shiny::tags$ol(
      shiny::tags$li("The default plot size is 16 × 10 cm. Change this by clicking the \"Customise plot layout\" button at the top of the options list. You may also add additional sub plots here."),
      shiny::tags$li("Set the ppm axis range as desired (typically slightly smaller than the processing range)."),
      shiny::tags$li("Adjust the offset between scans. This can be considered akin to the \"angle\" the plot is viewed at: 0 is equivalent to viewing along the time axis, ∞ to viewing down the intensity axis. Typically this depends on the number of scans, with more scans needing a larger offset."),
      shiny::tags$li("Adjust the upper y limit as desired."),
      shiny::tags$li("Select a colouring option, choices are:"),
      shiny::tags$ul(
        shiny::tags$li('Intensity based colouring. This is the defualt choice and generally looks good for most data.'),
        shiny::tags$ol(
          shiny::tags$li('Restrict colouring to be within the x-range of data that is changing as a function of time by changing the left and right axis limits.'),
          shiny::tags$li('Adjust the intensity scale to cover the y-range of data that is changing using the upper and lower intensity limits.'),
          shiny::tags$li('The out of bounds colour corresponds to intensities that fall outside this range'),
          shiny::tags$li('The default line colour corresponds to points that fall outside the left and right axis limits.')
        ),
        shiny::tags$li('Shading. This fills in the gap between a scan and its baseline, adding a 3D effect when looking down on the data from above. Typically only looks good if the plot offset is high.'),
        shiny::tags$ol(
      ...
      shiny::tags$li("Adjust the margins to ensure all axis labels are visible with minimal whitespace at the edges of the image, and the separation between plots looks good.")
    ),
    shiny::p('There are other options not described here that can also be adjusted if desired. Once the plot looks good, export the R script and run it to produce the plot.')
  )
}
...
interactive_plotting_mod <- function(input, output, session, data, data_name, visible=function() TRUE, ...) {
  ...
  output$scanTbl <- DT::renderDataTable({
    ...
    colnames(df) <- rep.int(c('Scan', htmltools::HTML(jms.classes:::expressionToHTML(xlab(echem_data())))), display_cols)
    table <- DT::datatable(
      df,
      class="compact hover",
      escape=FALSE,
  ...
  shiny::observeEvent(input$useCustomLayout, {
    shiny::showModal(
      shiny::modalDialog(size='l',
        shiny::div(style='min-height: 600px;',
                   shiny::h3("Customise plot size"),
                   shiny::fluidRow(
                     shiny::column(
                       6,
                       shiny::numericInput(session$ns('plotWidth'), 'Width of plot (cm)', plotSize$width, 5, step=1, width='100%')
                     ),
                     shiny::column(
                       6,
                       shiny::numericInput(session$ns('plotHeight'), 'Height of plot (cm)', plotSize$height, 5, step=1, width='100%')
                     )
                   ),
                   shiny::h3("Customise plot layout"),
                   shiny::p('It is currently expected that the echem plot is the same height as the NMR plot and to its right.', style="text-align: justify;"),
                   shiny::p('Additional plots may be added, they will initially be shown as placeholders (with 0 margins) in the plot behind this window. You can customise these plots using the tabs at the top of the options list.', style="text-align: justify;"),
                   shiny::p('This tool does not support overlapping plots (e.g. insets). Any such layouts will need to be manually defined in the plotting script.', style="text-align: justify;"),
  ...
  insituPlot <- shiny::callModule(insitu_plot_mod, "insituPlot", data, data_name, shiny::reactive({customLayout$matrix}), visible)
  ...
  outputplot <- shiny::renderPlot({
    doPlot(final=FALSE)
    annotator$updatendc()
    opar <<- par(c('cex','cex.axis','cex.lab','cex.main','cex.sub','col','col.axis','col.lab','col.main','col.sub','crt','family','fg','font','font.axis','font.lab','font.main','font.sub','lend','lheight','ljoin','lmitre', 'lty','lwd','mex','pch','ps','pty','srt'))
  })
  ...
  clickers <- shiny::callModule(patchedClickablePlot, 'clickable', outputplot,
                                shiny::reactive({plotSize$width}), shiny::reactive({plotSize$height}),
                                overlayFunc=overlay)
...
png('Insitu_nmr_plot.png', width=%s, height=%s, units='cm', res=300)", plotSize$width, plotSize$height)
...
#### Insitu Plot ####', script, paste(deparse(customLayout$matrix), collapse=' '), input$ps, input$cex, input$cexaxis)
    }

    script = sprintf("%s\n\n%s\n\n", script, insituPlot$script())

    if(hasEchem()) {
      script = sprintf("%s# Echem Plot annotations\n\n%s\n\n# NMR Plot annotations\n\n# Activate the NMR plot\nset_nmr()\n\n%s",
                       script, annotator$scriptFunc(2), annotator$scriptFunc(1))
    } else {
      script = sprintf("%s# Annotations\n\n%s",
  ...
  shiny::setBookmarkExclude(c("helpButton", "copy_r", "export_png", "useCustomLayout", "modalok", "plotOptionsTabs"))
...
interactivePlotting <- function(nmr) {
  ...
  server <- function(input, output, session) {
    shiny::callModule(interactive_plotting_mod, "plot", shiny::reactive({nmr}), shiny::reactive({data_name}))
  }
