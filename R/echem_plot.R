...
#' @param alignment Alignment parameters from plotNMR2D
...
plotechem_vertical <-function(data,V_range, alignment=NA, show_axes=TRUE,V_tick_interval=1,V_minor_tick_interval=0.25,time_tick_interval=5,time_ticks_dps=0,tick_size_multiplier=1) {
  ...
  #blank plot
  plot(0,0, type="n", xaxs="i", yaxs="i", xlim=c(-V_range[[2]],-V_range[[1]]), ylim=alignment$yrange, axes = FALSE, xlab = NA, ylab = NA)
  
  #plot echem
  lines(-data$Voltage.V,data$Test_Time.s.*(alignment$offset_scan_last-alignment$offset_scan_1)/(max(data$Test_Time.s.)-min(data$Test_Time.s.)))
  
  time_ticks_at=seq(min(data$Test_Time.s.),max(data$Test_Time.s.),time_tick_interval*3600)
  V_ticks_at=seq(V_range[[1]],V_range[[2]],V_tick_interval)*-1
  V_minor_ticks_at=seq(V_range[[1]],V_range[[2]],V_minor_tick_interval)*-1
  #scale ticks to correct coordinate system
  ...
  if(show_axes) {
      ...
      axis(side = 1, tck = -.015*tick_size_multiplier,at=V_ticks_at, labels = NA)
      axis(side = 1, tck = -.005*tick_size_multiplier, labels = NA,at=V_minor_ticks_at)
      ...
      ##Add axis titles
      mtext(side = 1, "Voltage (V)", line = 1.8)
      mtext(side = 4, "Time (h)", line = 2)
  }
}
