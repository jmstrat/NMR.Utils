...
patchedClickablePlot <- function(input, output, session, plotRenderer, width, height, overlayFunc=defaultOverlay) {
  ...
  output$plotUI <- shiny::renderUI({
    shiny::div(style=sprintf('position:relative; width: %scm; height: %scm', width(), height()),
               shiny::div(
               ...
               shiny::div(
                 ...
                 shiny::plotOutput(session$ns('overlay'), width=paste0(width(), 'cm'), height=paste0(height(), 'cm'),
